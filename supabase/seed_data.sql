...
BEGIN
    ...
    -- though random IDs are used here. Ideally, checking by name would be better
    -- but for a simple seeding block with random IDs, we just insert.
    -- To allow re-runnability without duplicates by name, we could check,
    -- but sticking to the simple insert as requested.
    ...
    -- Queen 1 (History): "GRN-01" (Year 2022)
    v_queen1_id := uuid_generate_v4();
    INSERT INTO queens (id, owner_id, hive_id, year, marking_code, lineage, breeder_name, status)
    VALUES (v_queen1_id, v_user_id, v_hive1_id, 2022, 'GRN-01', 'Carnica', 'Breeder A', 'Archived');

    -- Inspections for Queen 1: 10 total (90% CALM, 10% NERVOUS)
    -- 9 CALM
    FOR i IN 1..9 LOOP
    ...
    -- 1 NERVOUS
    INSERT INTO inspections (id, hive_id, queen_id, inspection_date, mood, colony_strength, pests_detected, user_id)
    VALUES (uuid_generate_v4(), v_hive1_id, v_queen1_id, '2023-05-01 10:00:00', 'NERVOUS', 'STRONG', '{}', v_user_id);

    -- Queen 2 (Current): "BLU-01" (Year 2024)
    ...
    -- Inspections for Queen 2: 5 total (100% CALM)
    -- First 4 standard
    FOR i IN 1..4 LOOP
        INSERT INTO inspections (id, hive_id, queen_id, inspection_date, mood, colony_strength, pests_detected, user_id)
    ...
    -- 5th Inspection (Latest) - Medication Check (Step 5)
    ...
    -- Inspections for Queen 4: 5 total (100% AGGRESSIVE)
    FOR i IN 1..5 LOOP
        INSERT INTO inspections (id, hive_id, queen_id, inspection_date, mood, colony_strength, pests_detected, user_id)
