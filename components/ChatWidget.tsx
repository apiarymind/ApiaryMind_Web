...
export default function ChatWidget() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 1. Dodaj wiadomość użytkownika od razu
    const userMsg: Message = { role: 'user', content: input };
    setMessages(prev => [...prev, userMsg]);
    setInput("");
    setIsLoading(true);

    // 2. SYMULACJA (Brak fetch/axios = Brak błędu network error)
    setTimeout(() => {
    ...
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end font-sans">
      ...
      {isOpen && (
        <div className="mb-4 w-[90vw] max-w-[380px] h-[500px] rounded-3xl overflow-hidden shadow-2xl border backdrop-blur-xl flex flex-col
          ...
          {/* POLE WPISYWANIA - STRZAŁKA (Zamiast rakiety) */}
